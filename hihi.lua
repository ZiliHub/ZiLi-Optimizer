-- ==========================================
-- 🏹 SCRIPT: AUTO CUPID (V3 - REWRITE)
-- Game: GET BETTER OUT | Cupid Dungeon
-- Fixes: Replay loop scope bug, uniqueID tracking bug, webhook
-- ==========================================
...
-- ==========================================
-- [0] KHỞI TẠO SESSION ID (PHẢI ĐỨNG ĐẦU)
-- BUG GỐC: currentScriptID được khai báo MUỘN hơn các task.spawn
-- → closure capture global nil thay vì local → loop thoát ngay lập tức
-- FIX: khai báo sớm, trước tất cả task.spawn
-- ==========================================
...
local currentScriptID = _G.DungeonScriptID  -- ← khai báo TRƯỚC mọi task.spawn
...
print("▶️ Đã khởi chạy Auto Cupid V3 (REWRITE)! Session ID: " .. currentScriptID)
...
-- ==========================================
-- [THÊM VÀO]: SPAM TAKESTAM LIÊN TỤC 24/7
-- ==========================================
task.spawn(function()
...
-- ==========================================
-- [4] BIẾN TOÀN CỤC SESSION
-- ==========================================
local DungeonStartTime     = tick()
local DungeonClearTimeStr  = "00:00"
local SessionItems         = {}
local ProcessedItems       = {}
local ProcessedLabels      = setmetatable({}, {__mode = "k"})  -- weak-key: tự GC khi label destroy
...
local function SendWebhook()
    ...
    for _, item in ipairs(SessionItems) do
        if item:match("%[ RARE ITEM %]") then
            table.insert(rareDrops, item)
            shouldPing = true
        elseif item:match("VIP Fruit") then
            table.insert(vipDrops, item)
            shouldPing = true
        else
    ...
    local colorHex = shouldPing and 0xFF2222 or 0xFFB347

    local embedTitle
    if #rareDrops > 0 and #vipDrops > 0 then
        embedTitle = "🔥  RARE + Fruit DROPPED !!!"
    elseif #rareDrops > 0 then
        embedTitle = "✨  RARE ITEM DROPPED !!!"
    elseif #vipDrops > 0 then
        embedTitle = "🍑  VIP FRUIT DROPPED !!!"
    else
        embedTitle = "🎁  Dungeon Cleared"
    end
    ...
    local payload = {
        embeds = {{
            ...
            footer      = {
                text     = "ZiLi Hub  •  " .. os.date("%d/%m/%Y  %H:%M:%S"),
                icon_url = LogoZiLi,
            },
...
-- ==========================================
-- [7] RADAR TRACK ĐỒ (FIX uniqueID BUG)
-- BUG GỐC: tostring(label) trả về TÊN instance ("TextLabel"), không unique
-- → nhiều label cùng tên → cùng uniqueID → bỏ sót drop
-- FIX: ProcessedLabels[label][itemName] — mỗi instance có bảng riêng
-- ==========================================
task.spawn(function()
...
-- ==========================================
-- [8] AUTO REPLAY
-- BUG GỐC: task.spawn ở line 175 script gốc, currentScriptID ở line 272
-- → closure đọc global currentScriptID = nil → while nil==1 = false → loop chết ngay
-- FIX: currentScriptID khai báo ở [0], trước mọi task.spawn
-- ==========================================
task.spawn(function()
    ...
    while _G.DungeonScriptID == currentScriptID do
        if _G.AutoDungeon and not isReplaying then
            pcall(function()
                -- Tìm ConfirmationPrompt trong PlayerGui
                ...
                if prompt then
                    ...
                    local btn     = options and (
                        options:FindFirstChild("Replay")     or
                        options:FindFirstChild("PlayAgain")  or
                        options:FindFirstChild("Again")
                    )
                    ...
                    if btn then
                        ...
                        if isVisible then
                            isReplaying = true
                            print("⏳ Bảng Replay đã hiện! Đợi 1.5s để Server chuẩn bị...")
                            task.wait(1.5)

                            local val    = btn:GetAttribute("buttonValue") or btn.Name
                            local remote = prompt:FindFirstChild("RemoteEvent")

                            -- Thử cách 1: RemoteEvent trong prompt
                            if remote then
                                pcall(function() remote:FireServer(val) end)
                                print("🚀 Replay (UI Remote) → val:", tostring(val))
                            else
                                -- Thử cách 2: nil instances
                                local fired = false
                                ...
                                -- Thử cách 3: Click trực tiếp vào nút qua VIM
                                if not fired then
                                    pcall(function()
    ...
    print("🔴 Replay loop thoát — Session ID cũ:", currentScriptID)
end)
        ...
        for _, track in pairs(animator:GetPlayingAnimationTracks()) do
            if track.Weight > 0.01 and track.Animation then
                ...
                if not (animName:match("idle") or animName:match("walk") or animName:match("run") or animName:match("stun") or animName:match("hit")) then
                    if animName:match("attack") or animName:match("slash")  or animName:match("punch")  or
                       animName:match("swing")  or animName:match("cast")   or animName:match("skill")  or
                       animName:match("m1")     or animName:match("slam")   or animName:match("smash")  or
                       animName:match("charge") or animName:match("burst")  or animName:match("combo")  or
                       animName:match("shoot")  or animName:match("fire")   or animName:match("gun")    or
                       animName:match("ranged") or animName:match("magic")  or animName:match("throw")  or
                       animName:match("shot") then
                    ...
                    if workspace:FindFirstChild("Effects") then
                            ...
                            for _, v in pairs(workspace.Effects:GetChildren()) do
                                ...
                                if not IgnoredHazards[v] then
                                    if n:match("enkai") then
                                        bossHazardDist = 100; bossHazardInst = v; break
                                    elseif n:match("pillar") or n:match("flame") then
                                        bossHazardDist = 50; bossHazardInst = v; break
                                    end
                                end
                            end
                        end
                        ...
                        for _, v in pairs(workspace.Effects:GetChildren()) do
                            ...
                            if vPos then
                                ...
                                if not IgnoredHazards[v]
                                and (name:match("aoe") or name:match("circle") or name:match("bomb") or name:match("meteor")
                                  or name:match("lightning") or name:match("arrow") or name:match("rain") or name:match("projectile"))
                                and dist < minDist and detectedHazard ~= "BossSkill" then
            ...
            pcall(function()
                ...
                if root then
                    if CurrentZoneIndex >= 7 and CurrentZoneIndex <= 8 then
                        if #GetMobsInZone(root.Position) > 0 then shouldBlock = true end
            ...
            if char and char.Parent then
                pcall(function()
                    ...
                    if #enemiesToHit > 0 and primaryCFrame then
                        ...
                        if CurrentZoneIndex >= 7 and CurrentZoneIndex <= 8 then _G.ForceReblock = true end
