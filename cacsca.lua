...
__modules["Farm/AutoFishMerchant"] = function()
    ...
    function Tween.Start(finalDest, onComplete, opts)
        ...
        local function flyTo(stepData, onStepComplete)
            ...
            local STALL_TIMEOUT = 3   -- giây không di chuyển → force snap
            task.spawn(function()
    ...
    local function AutoCraftSilent(blueprintType, extraDataKey, fishList, minCount, countPerCraft)
        ...
        for _, fishName in ipairs(fishList) do
            ...
            if count >= minCount then
                ...
                for _ = 1, batches do
                    if not _G.AutoFishing then return false end
                    -- [OPTIMIZE] Dùng cached CraftingRemoteR thay vì WaitForChild mỗi lần
                    if CraftingRemoteR then
                        pcall(function()
                            CraftingRemoteR:InvokeServer(unpack({{
                                ["BlueprintItem"]=blueprintType, ["Method"]="Craft",
                                ["ExtraData"]={[extraDataKey]=fishName}, ["Count"]=1,
                                -- [FIX CRAFT RARE] Count=1 = craft 1 bait per call.
                                -- Dùng countPerCraft=2 sẽ yêu cầu server craft 2 bait (4 cá)
                                -- trong khi chỉ có 2 cá → server từ chối, pcall nuốt lỗi,
                                -- loop cứ bay lại craft mãi mà không craft được gì.
                            }}))
                        end)
                    end
                    task.wait(0.5)
...
AutoRejoinModule._SESSION_FILE = "gbo_session.json"
...
function AutoRejoinModule._saveSession()
    pcall(function()
        ...
        if ok then writefile(AutoRejoinModule._SESSION_FILE, js) end
    end)
end
...
function AutoRejoinModule.Start()
    ...
    -- Write session immediately so kick mid-session still restores
    AutoRejoinModule._saveSession()

    if not AutoRejoinModule._hooked then
        AutoRejoinModule._hooked = true

        -- Signal 1: OnTeleport Failed (teleport kicked)
        pcall(function()
            Player_L.OnTeleport:Connect(function(teleportState)
        ...
        -- Signal 2: Game:GetService("Players").PlayerRemoving = lobby forced disconnect
        pcall(function()
            game:GetService("Players").PlayerRemoving:Connect(function(p)
                if p == Player_L and AutoRejoinModule._running then
                    AutoRejoinModule._saveSession()
                end
            end)
        end)
    ...
    -- Background: keep session file fresh every 20s + watch for kick
    AutoRejoinModule._thread = task.spawn(function()
        local lastCharTime = os.time()
        while AutoRejoinModule._running do
            task.wait(6)
            -- Keep file up to date (handles sudden kills / crashes)
            AutoRejoinModule._saveSession()
            -- Character watch: nil for >12s after previously existing = kick
            if Player_L.Character then
                lastCharTime = os.time()
            elseif os.time() - lastCharTime > 12 then
                if AutoRejoinModule._running then
                    task.wait(3)
                    AutoRejoinModule._doRejoin()
                end
...
function AutoRejoinModule.Stop()
    ...
    -- Clear session file when user turns off rejoin
    pcall(function()
        if deletefile and isfile and isfile(AutoRejoinModule._SESSION_FILE) then
            deletefile(AutoRejoinModule._SESSION_FILE)
        end
    end)
end
...
local function DrawIcon(parent, iconName, px, py, sz, col)
    ...
    elseif iconName=="server" then
        -- Server tower: rounded chassis + slot lines + large status LED
        -- Outer chassis (full body)
        local chassis = NEW("Frame",{
            Size=UDim2.new(0,s*.86,0,s*.90),
            Position=UDim2.new(0,s*.07,0,s*.05),
            BackgroundTransparency=1, BorderSizePixel=0
        }, c)
        CORNER(s*.10, chassis)
        STROKE(col, 1.8, 0, chassis)
        -- 3 drive bay lines inside chassis
        L(s*.18,s*.30, s*.68,s*.30, 1.4)
        L(s*.18,s*.52, s*.68,s*.52, 1.4)
        L(s*.18,s*.74, s*.68,s*.74, 1.4)
        -- Status LED: large colored dot, named SDot, always visible
        local d1 = Dot(s*.80,s*.29, s*.14, GREEN)
        if d1 then d1.Name="SDot"; d1:SetAttribute("DotColor","GREEN") end
        local d2 = Dot(s*.80,s*.51, s*.14, AMBER)
        if d2 then d2.Name="SDot"; d2:SetAttribute("DotColor","AMBER") end
        local d3 = Dot(s*.80,s*.73, s*.14, RED)
...
if IS_LOBBY then
    ...
    local psCard = MakeCard(PrivateServerPage, 390, 1)
    ...
    NEW("TextLabel",{
        Text="Leave empty for public server",
        Size=UDim2.new(1,-24,0,12), Position=UDim2.new(1,-196,0,37),
        BackgroundTransparency=1, TextColor3=TEXT3,
        Font=Enum.Font.Gotham, TextSize=9, TextXAlignment=Enum.TextXAlignment.Right
    }, psCard)

    local psBg = NEW("Frame",{Size=UDim2.new(1,-24,0,32),Position=UDim2.new(0,12,0,52),BackgroundColor3=BG5,BorderSizePixel=0},psCard)
    CORNER(7,psBg); STROKE(GOLD3,1,0.45,psBg)
    local psBox = NEW("TextBox",{
        ...
        TextColor3=TEXT1,PlaceholderColor3=TEXT3,Font=Enum.Font.Gotham,TextSize=11,
        TextXAlignment=Enum.TextXAlignment.Left,ClearTextOnFocus=false
    },psBg)
    psBox.Focused:Connect(function() TWEEN(psBg:FindFirstChildOfClass("UIStroke"),0.15,{Color=PINK}) end)
    psBox.FocusLost:Connect(function() TWEEN(psBg:FindFirstChildOfClass("UIStroke"),0.15,{Color=GOLD3}) end)
    ...
    local HUB_GRID = {
        {id="Regular",       label="Regular",       icon="globe",  col=COL_TRAVEL, gx=0, gy=0},
        {id="Trade Hub",     label="Trade Hub",     icon="coin",   col=AMBER,      gx=1, gy=0},
        {id="Universe Hub",  label="Universe Hub",  icon="target", col=COL_STATS,  gx=0, gy=1},
        {id="Fish Hub",      label="Fish Hub",      icon="fish",   col=COL_FISH,   gx=1, gy=1},
    }
    local HUB_CELL_W = (1/2)
    local HUB_START_Y = 118

    local function UpdateHubSelection()
        for _,hd in ipairs(HUB_GRID) do
            local d = HubButtons[hd.id]
            if not d then continue end
            local sel = (hd.id == getgenv().SelectedHub)
            local ac = hd.col
            local acd = C(
                math.min(255,math.floor(ac.R*255*0.15+5)),
    ...
    for _,hd in ipairs(HUB_GRID) do
        local cellPad = 5
        local cellW_abs = (psCard.AbsoluteSize.X > 0 and psCard.AbsoluteSize.X or 310) / 2 - cellPad * 1.5
        local hBtn = NEW("TextButton",{
            Size     = UDim2.new(0.5,-cellPad-3, 0, 46),
            Position = UDim2.new(hd.gx*0.5, hd.gx==0 and cellPad or cellPad/2,
                                 0, HUB_START_Y + hd.gy*52),
        ...
        -- Icon
        local hIconBg = NEW("Frame",{
            Size=UDim2.new(0,22,0,22), Position=UDim2.new(0,8,0.5,-11),
            BackgroundColor3=BG5, BorderSizePixel=0
        }, hBtn)
        CORNER(6, hIconBg)
        DrawIcon(hIconBg, hd.icon, 4, 4, 14, TEXT3)
        ...
        hBtn.MouseEnter:Connect(function()
            if hd.id ~= getgenv().SelectedHub then
                TWEEN(hBtn,0.12,{BackgroundTransparency=0.15,BackgroundColor3=BG4})
            end
        end)
        hBtn.MouseLeave:Connect(function()
            if hd.id ~= getgenv().SelectedHub then
                TWEEN(hBtn,0.12,{BackgroundTransparency=0.3})
            end
        end)
    ...
    -- ── Sea selector (2 radio buttons, only for Regular) ──────────
    local seaY = HUB_START_Y + 2*52 + 4
    RowDivider(psCard, seaY - 2)
    local seaHeaderLbl = NEW("TextLabel",{
    ...
    local SEA_OPTS = {
        {id="Sea 1", label="⚓  SEA 1", key="First Sea"},
        {id="Sea 2", label="🌊  SEA 2", key="Second Sea"},
    }
    local SeaBtns = {}

    local function UpdateSeaSelection()
        for _,sd in ipairs(SEA_OPTS) do
            local d = SeaBtns[sd.id]
            if not d then continue end
            local sel = (getgenv().SelectedSea == sd.id)
            TWEEN(d.Btn, 0.15, {
                BackgroundColor3 = sel and C(8,18,44) or BG4,
                BackgroundTransparency = sel and 0 or 0.3
            })
            TWEEN(d.Strk, 0.15, {
                Color = sel and COL_FISH or C(35,35,55),
    ...
    for si, sd in ipairs(SEA_OPTS) do
        local sb = NEW("TextButton",{
            ...
            Position=UDim2.new((si-1)*0.5, si==1 and 5 or 3, 0, seaY+22),
            BackgroundColor3=BG4, BackgroundTransparency=0.3,
            Text="", AutoButtonColor=false
        }, psCard)
        CORNER(8, sb)
    ...
    -- ── Auto Join (persistent preference toggle) ──────────────────
    local ajY = seaY + 62
    RowDivider(psCard, ajY)
    NEW("TextLabel",{
        Text="AUTO JOIN",
        Size=UDim2.new(0.55,0,0,16), Position=UDim2.new(0,12,0,ajY+8),
        BackgroundTransparency=1, TextColor3=TEXT1,
        Font=Enum.Font.GothamBold, TextSize=12, TextXAlignment=Enum.TextXAlignment.Left
    }, psCard)
    NEW("TextLabel",{
        Text="Stays ON · auto-joins when lobby starts",
        Size=UDim2.new(1,-24,0,12), Position=UDim2.new(0,12,0,ajY+26),
        BackgroundTransparency=1, TextColor3=TEXT3,
        Font=Enum.Font.Gotham, TextSize=9, TextXAlignment=Enum.TextXAlignment.Left
    }, psCard)

    -- Persistent toggle: does NOT auto-off
    local ajPill = NEW("TextButton",{
        Size=UDim2.new(0,48,0,26), Position=UDim2.new(1,-60,0,ajY+12),
        BackgroundColor3=BG5, Text="", AutoButtonColor=false
    }, psCard)
    CORNER(20, ajPill)
    ...
    local function SetAutoJoin(on)
        ...
        local ac, ad = COL_PS, PINKD
        TWEEN(ajPill,  0.22, {BackgroundColor3 = on and ad or BG5})
        TWEEN(ajStrk,  0.22, {Color = on and ac or TEXT3, Transparency = on and 0 or 0.3})
        TWEEN(ajThumb, 0.22, {
            BackgroundColor3 = on and ac or TEXT3,
            Position = on and UDim2.new(1,-22,0.5,-9) or UDim2.new(0,4,0.5,-9)
        })
    ...
    local joinNowBtn = NEW("TextButton",{
        Size=UDim2.new(1,-24,0,34), Position=UDim2.new(0,12,0,jnY),
        BackgroundColor3=PINKD, Text="", AutoButtonColor=false
    }, psCard)
    CORNER(9, joinNowBtn)
    STROKE(PINK, 1.5, 0.2, joinNowBtn)

    -- Icon + label inside JOIN NOW
    DrawIcon(joinNowBtn, "server", 14, 7, 20, PINK)
    NEW("TextLabel",{
        Text="JOIN NOW",
    ...
    NEW("TextLabel",{
        Text="One-shot immediate join",
        Size=UDim2.new(0.55,0,0,12), Position=UDim2.new(0,40,1,-14),
    ...
    joinNowBtn.MouseButton1Click:Connect(function()
        ...
        task.wait(0.1)
        TWEEN(joinNowBtn, 0.15, {BackgroundColor3=PINKD})
        task.spawn(DoJoin)
    end)

    -- Resize psCard to fit content
    task.spawn(function()
        task.wait(0.05)
        psCard.Size = UDim2.new(1,-24, 0, jnY + 50)
    end)

    -- ── Config_SelectedHub restore callback ───────────────────────
    ...
    -- Default: Regular + Sea 1 selected
    task.spawn(function()
        task.wait(0.1)
        UpdateHubSelection()
        UpdateSeaSelection()
    ...
    -- ── LOBBY AUTO-JOIN — reads session file on startup ────────────
    task.spawn(function()
        task.wait(2.2)  -- wait for lobby to fully load
        local SESSION_FILE = "gbo_session.json"
        local pending = getgenv().GBO_PendingJoin

        -- Read from session file (survives executor getgenv reset)
        if not pending then
            pcall(function()
                if isfile and isfile(SESSION_FILE) then
                    local hs = game:GetService("HttpService")
                    local ok, data = pcall(function()
                        return hs:JSONDecode(readfile(SESSION_FILE))
                    end)
                    if ok and type(data)=="table" and data.autoRejoin then
                        pending = data
                        pcall(function() deletefile(SESSION_FILE) end)
                    end
                end
            end)
        end
        -- Backwards compat: old pending join file
        if not pending then
            pcall(function()
        ...
        -- Multi-account: check if THIS account has a slot configured
        -- (multi-acc takes lower priority than session file, but higher than nothing)
        if not pending then
            pcall(function()
                if not isfile or not isfile("gbo_multiacc.json") then return end
                local hs = game:GetService("HttpService")
                local ok, mData = pcall(function()
                    return hs:JSONDecode(readfile("gbo_multiacc.json"))
                end)
                if not ok or type(mData) ~= "table" then return end

                -- Try exact username match first, then DisplayName
                local myName    = LocalPlayer.Name
                local myDisplay = LocalPlayer.DisplayName
                local slot = mData[myName] or mData[myDisplay]

                if slot and type(slot)=="table" then
                    pending = {
                        code       = slot.code or "",
                        hub        = slot.hub  or "Regular",
                        sea        = slot.sea  or "Sea 1",
                        config     = slot.config,
                        autoRejoin = true,
                        fromMultiAcc = true,
                    }
        ...
        if pending and type(pending)=="table" then
            ...
            -- NOTE: config auto-load removed — player sets auto load manually in Config tab
            ...
            -- Execute join using InvokeServer (skip if no code = public)
            if pCode ~= "" then
                task.wait(0.5)
                pcall(function()
                    ReplicatedStorage_L:WaitForChild("Events")
                ...
                -- Hub selection after joining
                task.spawn(function()
                    local _arg = HubArgs[pHub] or true
                    if pHub == "Regular" then _arg = true end
                    local pGui = Player_L:WaitForChild("PlayerGui")
                    local chooseTypeUI = pGui:WaitForChild("chooseType", 20)
                    if chooseTypeUI then
                        local frame = chooseTypeUI:WaitForChild("Frame", 8)
                        if frame then
                            local remote = frame:WaitForChild("RemoteEvent", 8)
                            if remote then
                                task.wait(0.6)
                                pcall(function() remote:FireServer(_arg) end)
                                pcall(function() chooseTypeUI.Enabled = false end)
                            end
                        end
                    end
    ...
    -- ── AUTO REJOIN ───────────────────────────────────────────────────
    local rejoinCard = MakeCard(PrivateServerPage, 90, 2)
    CardHeader(rejoinCard, "lightning", "AUTO REJOIN", CYAN)
    RowLabel(rejoinCard, "Auto Rejoin", "Saves session · auto-rejoins after kick", 34)
    ...
    RowDivider(rejoinCard, 74)
    NEW("TextLabel",{
        Text="When ON: writes session file every 20s. Lobby reads it on restart and auto-joins.",
        Size=UDim2.new(1,-24,0,14), Position=UDim2.new(0,12,0,76),
        BackgroundTransparency=1, TextColor3=TEXT3,
        Font=Enum.Font.Gotham, TextSize=9, TextXAlignment=Enum.TextXAlignment.Left,
        TextWrapped=true
    }, rejoinCard)
    ...
    -- ── MULTI-ACCOUNT MANAGER (Lobby) ─────────────────────────────────
    -- Same gbo_multiacc.json used by game world; lobby reads it on startup
    ...
    local lbMaccCard = MakeCard(PrivateServerPage, 10, 3) -- height set dynamically
    CardHeader(lbMaccCard, "user", "MULTI-ACCOUNT (VIEW)", PURPLE)
    NEW("TextLabel",{
        Text="Slots configured in-game · this account auto-joins on lobby start if found",
        Size=UDim2.new(1,-24,0,24), Position=UDim2.new(0,12,0,36),
        BackgroundTransparency=1, TextColor3=TEXT3,
        Font=Enum.Font.Gotham, TextSize=9, TextXAlignment=Enum.TextXAlignment.Left,
        TextWrapped=true
    }, lbMaccCard)
    ...
    local lbMaccScroll = NEW("ScrollingFrame",{
        Size=UDim2.new(1,-24,0,110), Position=UDim2.new(0,12,0,66),
        BackgroundColor3=BG0, BorderSizePixel=0,
        ScrollBarThickness=2, ScrollBarImageColor3=PURPLE,
        AutomaticCanvasSize=Enum.AutomaticSize.Y, CanvasSize=UDim2.new(0,0,0,0)
    }, lbMaccCard)
    CORNER(8,lbMaccScroll); STROKE(C(50,20,80),1,0.3,lbMaccScroll)
    NEW("UIListLayout",{
        SortOrder=Enum.SortOrder.LayoutOrder,
        Padding=UDim.new(0,3),
        HorizontalAlignment=Enum.HorizontalAlignment.Center
    }, lbMaccScroll)
    NEW("UIPadding",{PaddingTop=UDim.new(0,4),PaddingBottom=UDim.new(0,4)}, lbMaccScroll)

    -- Current account badge
    local myName = LocalPlayer.Name
    local myBadge = NEW("TextLabel",{
        Text="This account: "..myName,
    ...
    local mySlotLbl = NEW("TextLabel",{
        Text="No slot configured for this account",
        Size=UDim2.new(1,-24,0,14), Position=UDim2.new(0,12,0,197),
        BackgroundTransparency=1, TextColor3=TEXT3,
        Font=Enum.Font.Gotham, TextSize=9, TextXAlignment=Enum.TextXAlignment.Left
    }, lbMaccCard)

    -- Refresh button
    local lbRefreshBtn = NEW("TextButton",{
        Text="⟳  REFRESH",
        Size=UDim2.new(1,-24,0,26), Position=UDim2.new(0,12,0,216),
        BackgroundColor3=C(22,8,44), TextColor3=PURPLE,
        Font=Enum.Font.GothamBold, TextSize=10, AutoButtonColor=false
    }, lbMaccCard)
    CORNER(7,lbRefreshBtn); STROKE(PURPLE,1.2,0.2,lbRefreshBtn)

    local function LbMAccRefresh()
        -- Clear list
        for _,ch in ipairs(lbMaccScroll:GetChildren()) do
        ...
        for accName, slot in pairs(lbMaccData) do
            ...
            local row = NEW("Frame",{
                Size=UDim2.new(1,-6,0,40), BackgroundColor3= isMe and C(18,8,36) or BG3,
                LayoutOrder=idx, BorderSizePixel=0
            }, lbMaccScroll)
            CORNER(6,row)
            local rowStrk = STROKE(isMe and PURPLE or C(50,20,80),1,isMe and 0.2 or 0.5,row)

            NEW("TextLabel",{
                Text="  👤  "..accName..(isMe and "  ← YOU" or ""),
                Size=UDim2.new(1,0,0,18), Position=UDim2.new(0,0,0,2),
                BackgroundTransparency=1,
                TextColor3= isMe and GOLD2 or TEXT1,
                Font=Enum.Font.GothamBold, TextSize=11,
                TextXAlignment=Enum.TextXAlignment.Left
            }, row)

            local hub    = slot.hub    or "Regular"
            local sea    = slot.sea    or "Sea 1"
            local config = slot.config or "(none)"
            local code   = slot.code   or ""
            local codeShort = code=="" and "Public" or (string.sub(code,1,10).."…")
            NEW("TextLabel",{
                Text="  "..hub.." · "..sea.." · cfg:"..config.." · "..codeShort,
        ...
        if idx == 0 then
            NEW("TextLabel",{
                Text="No multi-acc slots found · set them up in-game",
                Size=UDim2.new(1,-12,0,36), LayoutOrder=1,
                BackgroundTransparency=1, TextColor3=TEXT3,
                Font=Enum.Font.GothamMedium, TextSize=10,
                TextXAlignment=Enum.TextXAlignment.Center
            }, lbMaccScroll)
        end
        -- Update this-account slot label
        local mySlot = lbMaccData[myName] or lbMaccData[LocalPlayer.DisplayName]
        if mySlot then
            local h = mySlot.hub or "Regular"
            local s = mySlot.sea or "Sea 1"
            local cf = mySlot.config or "(none)"
            local co = mySlot.code or ""
            mySlotLbl.Text = "Hub:"..h.." · "..s.." · cfg:"..cf
            mySlotLbl.TextColor3 = GREEN
            -- Apply settings to UI
            if co ~= "" then
                getgenv().PSCode = co
                pcall(function() psBox.Text = co end)
            end
            getgenv().SelectedHub = h
            getgenv().SelectedSea = s
            UpdateHubSelection()
            UpdateSeaSelection()
        else
    ...
    LbMAccRefresh()
    lbRefreshBtn.MouseButton1Click:Connect(LbMAccRefresh)
    lbRefreshBtn.MouseEnter:Connect(function() TWEEN(lbRefreshBtn,0.12,{BackgroundColor3=C(32,12,58)}) end)
    lbRefreshBtn.MouseLeave:Connect(function() TWEEN(lbRefreshBtn,0.12,{BackgroundColor3=C(22,8,44)}) end)

    task.spawn(function()
        task.wait(0.05)
        lbMaccCard.Size = UDim2.new(1,-24, 0, 250)
    end)
    ...
    -- Matches lobby layout exactly; also has current server info + multi-acc
    PageLayout(PrivateServerPage, 14, 10)
    ...
    -- scoped helper to update toggle visuals without firing callbacks
    ...
    local gwStatusCard = MakeCard(PrivateServerPage, 68, 0)
    CardHeader(gwStatusCard, "shield", "CURRENT SERVER", CYAN)

    local isInPS = (game.PrivateServerId ~= "")
    local psIdShort = isInPS
        and (string.sub(game.PrivateServerId,1,8).."…"..string.sub(game.PrivateServerId,-6))
        or "Public Server"

    -- Server type badge
    local gwSrvBadge = NEW("TextLabel",{
        Text = isInPS and "✦  PRIVATE" or "◇  PUBLIC",
        Size=UDim2.new(0,82,0,18), Position=UDim2.new(1,-94,0,32),
        BackgroundColor3 = isInPS and C(10,30,52) or C(36,8,8),
        TextColor3 = isInPS and CYAN or RED,
        Font=Enum.Font.GothamBold, TextSize=9, TextXAlignment=Enum.TextXAlignment.Center
    }, gwStatusCard)
    CORNER(5, gwSrvBadge); STROKE(isInPS and CYAN or RED, 1, 0.4, gwSrvBadge)

    NEW("TextLabel",{
        Text = "ID: " .. psIdShort,
        Size=UDim2.new(1,-114,0,14), Position=UDim2.new(0,12,0,34),
        BackgroundTransparency=1, TextColor3=TEXT2,
        Font=Enum.Font.GothamMedium, TextSize=10, TextXAlignment=Enum.TextXAlignment.Left
    }, gwStatusCard)
    NEW("TextLabel",{
        Text = "JobId: " .. string.sub(game.JobId,1,12) .. "…",
        Size=UDim2.new(1,-114,0,12), Position=UDim2.new(0,12,0,50),
        BackgroundTransparency=1, TextColor3=TEXT3,
        Font=Enum.Font.Gotham, TextSize=9, TextXAlignment=Enum.TextXAlignment.Left
    }, gwStatusCard)

    -- Copy PS code button (copies game.PrivateServerAccessKey if available)
    local copyCdBtn = NEW("TextButton",{
        Text="⎘  COPY CODE",
        Size=UDim2.new(0,82,0,18), Position=UDim2.new(1,-94,0,50),
        BackgroundColor3=C(8,24,20), TextColor3=CYAN,
        Font=Enum.Font.GothamBold, TextSize=8,
        AutoButtonColor=false
    }, gwStatusCard)
    CORNER(5, copyCdBtn); STROKE(CYAN,1,0.5,copyCdBtn)
    copyCdBtn.MouseButton1Click:Connect(function()
        local code = ""
        pcall(function() code = game.PrivateServerAccessKey or game.PrivateServerId or "" end)
        if code ~= "" then
            pcall(function() setclipboard(code) end)
            copyCdBtn.Text = "✓  COPIED"
            task.delay(2, function()
                if copyCdBtn and copyCdBtn.Parent then copyCdBtn.Text = "⎘  COPY CODE" end
            end)
        end
    end)

    -- ── Check PS card ───────────────────────────────────────────────────
    local checkPsCard = MakeCard(PrivateServerPage, 92, 0)  -- between status(0) and setup(1)
    checkPsCard.LayoutOrder = 0.5  -- not int so we set explicitly after
    checkPsCard.LayoutOrder = 1
    -- bump setup card order
    gwStatusCard.LayoutOrder = 0
    checkPsCard.LayoutOrder  = 1
    CardHeader(checkPsCard, "target", "CHECK PRIVATE SERVER", COL_STATS)

    -- Current PS access key (the shareable code)
    local currentAccessKey = ""
    pcall(function() currentAccessKey = game.PrivateServerAccessKey or "" end)

    NEW("TextLabel",{
        Text = "Current PS Code:",
        Size=UDim2.new(0.45,0,0,14), Position=UDim2.new(0,12,0,36),
        BackgroundTransparency=1, TextColor3=TEXT2,
        Font=Enum.Font.GothamBold, TextSize=9, TextXAlignment=Enum.TextXAlignment.Left
    }, checkPsCard)

    local checkCodeLbl = NEW("TextLabel",{
        Text = currentAccessKey ~= "" and (string.sub(currentAccessKey,1,12).."…") or "(not in PS)",
        Size=UDim2.new(0.55,-8,0,14), Position=UDim2.new(0.45,0,0,36),
        BackgroundTransparency=1,
        TextColor3 = currentAccessKey ~= "" and CYAN or TEXT3,
        Font=Enum.Font.GothamMedium, TextSize=9, TextXAlignment=Enum.TextXAlignment.Left
    }, checkPsCard)

    RowDivider(checkPsCard, 54)

    -- Match status row
    local matchStatusLbl = NEW("TextLabel",{
        Text = "Match with entered code: —",
        Size=UDim2.new(1,-24,0,14), Position=UDim2.new(0,12,0,60),
        BackgroundTransparency=1, TextColor3=TEXT3,
        Font=Enum.Font.GothamBold, TextSize=10, TextXAlignment=Enum.TextXAlignment.Left
    }, checkPsCard)

    -- Refresh/check button
    local checkPsBtn = NEW("TextButton",{
        Text="⟳  CHECK",
        Size=UDim2.new(0,80,0,22), Position=UDim2.new(1,-94,0,57),
        BackgroundColor3=C(10,22,44), TextColor3=COL_STATS,
        Font=Enum.Font.GothamBold, TextSize=9, AutoButtonColor=false
    }, checkPsCard)
    CORNER(6, checkPsBtn); STROKE(COL_STATS,1,0.3,checkPsBtn)
    checkPsBtn.MouseButton1Click:Connect(function()
        local entered = getgenv().PSCode or ""
        local actual  = ""
        pcall(function() actual = game.PrivateServerAccessKey or "" end)
        -- Refresh display
        checkCodeLbl.Text = actual ~= "" and (string.sub(actual,1,16).."…") or "(not in PS)"
        checkCodeLbl.TextColor3 = actual ~= "" and CYAN or TEXT3
        if actual == "" then
            matchStatusLbl.Text = "⬡  Currently in a PUBLIC server"
            matchStatusLbl.TextColor3 = TEXT3
        elseif entered == "" then
            matchStatusLbl.Text = "⚠  No code entered — copy it below"
            matchStatusLbl.TextColor3 = AMBER
            -- Auto-fill entered code box with actual key
            pcall(function() gwPsBox.Text = actual end)
        elseif string.find(actual, entered, 1, true) or string.find(entered, actual, 1, true) then
            matchStatusLbl.Text = "✓  MATCH — code is correct"
            matchStatusLbl.TextColor3 = GREEN
        else
            matchStatusLbl.Text = "✕  MISMATCH — code differs"
            matchStatusLbl.TextColor3 = RED
        end
        TWEEN(checkPsBtn,0.1,{BackgroundColor3=C(18,34,64)})
        task.delay(0.3, function()
            if checkPsBtn and checkPsBtn.Parent then
                TWEEN(checkPsBtn,0.2,{BackgroundColor3=C(10,22,44)})
            end
        end)
    end)

    -- ── PS Setup card (same layout as lobby) ───────────────────────────    local gwPsCard = MakeCard(PrivateServerPage, 10, 2) -- height set dynamically
    CardHeader(gwPsCard, "server", "SERVER SETUP", PINK)
    ...
    NEW("TextLabel",{
        ...
        Size=UDim2.new(1,-24,0,12), Position=UDim2.new(1,-184,0,37),
        BackgroundTransparency=1, TextColor3=TEXT3,
        Font=Enum.Font.Gotham, TextSize=9, TextXAlignment=Enum.TextXAlignment.Right
    }, gwPsCard)
    ...
    CORNER(7, gwPsBg)
    local gwPsBgStrk = STROKE(GOLD3,1,0.45,gwPsBg)
    ...
    gwPsBox.Focused:Connect(function() TWEEN(gwPsBgStrk,0.15,{Color=PINK}) end)
    gwPsBox.FocusLost:Connect(function() TWEEN(gwPsBgStrk,0.15,{Color=GOLD3}) end)

    -- Sync with Config_PSCode (shared key used by ConfigManager)
    if not TogglesData["Config_PSCode"] then
        TogglesData["Config_PSCode"] = {
            Value = "", HeadBtn = gwPsBox,
            Callback = function(val)
                getgenv().PSCode = val or ""
    ...
    gwPsBox:GetPropertyChangedSignal("Text"):Connect(function()
        ...
        if TogglesData["Config_PSCode"] then TogglesData["Config_PSCode"].Value = gwPsBox.Text end
    end)

    -- Hub 2×2 grid
    RowDivider(gwPsCard, 92)
    NEW("TextLabel",{
        Text="Destination Hub",
        Size=UDim2.new(0.5,0,0,14), Position=UDim2.new(0,12,0,100),
        BackgroundTransparency=1, TextColor3=TEXT2,
    ...
    local GW_HUB_START_Y = 118

    local function GW_UpdateHubSelection()
        for _,hd in ipairs(GW_HUB_GRID) do
            local d = GW_HubButtons[hd.id]
            if not d then continue end
            local sel = (hd.id == getgenv().SelectedHub)
            local ac  = hd.col
            local acd = C(
    ...
    for _,hd in ipairs(GW_HUB_GRID) do
        local hBtn = NEW("TextButton",{
            ...
            Position = UDim2.new(hd.gx*0.5, hd.gx==0 and 5 or 3, 0, GW_HUB_START_Y + hd.gy*52),
            BackgroundColor3=BG4, BackgroundTransparency=0.3,
            Text="", AutoButtonColor=false
        }, gwPsCard)
        CORNER(9, hBtn)
        ...
        GW_HubButtons[hd.id] = {Btn=hBtn, Strk=hStrk, Lbl=hLbl, IconBg=hIconBg}
        hBtn.MouseEnter:Connect(function()
        ...
        hBtn.MouseButton1Click:Connect(function()
            getgenv().SelectedHub = hd.id
            GW_UpdateHubSelection()
        end)
    end

    -- Sea radio buttons
    local gwSeaY = GW_HUB_START_Y + 2*52 + 4
    RowDivider(gwPsCard, gwSeaY - 2)
    local gwSeaHeaderLbl = NEW("TextLabel",{
        Text="Sea — Regular Only",
        Size=UDim2.new(0.55,0,0,14), Position=UDim2.new(0,12,0,gwSeaY+4),
        BackgroundTransparency=1, TextColor3=TEXT2,
        Font=Enum.Font.GothamBold, TextSize=9, TextXAlignment=Enum.TextXAlignment.Left
    }, gwPsCard)
    ...
    local function GW_UpdateSeaSelection()
        for _,sd in ipairs(GW_SEA_OPTS) do
            local d = GW_SeaBtns[sd.id]
            if not d then continue end
            local sel = (getgenv().SelectedSea == sd.id)
    ...
    for si, sd in ipairs(GW_SEA_OPTS) do
        local sb = NEW("TextButton",{
            Size=UDim2.new(0.5,-8,0,30),
            Position=UDim2.new((si-1)*0.5, si==1 and 5 or 3, 0, gwSeaY+22),
            BackgroundColor3=BG4, BackgroundTransparency=0.3,
            Text="", AutoButtonColor=false
        }, gwPsCard)
        CORNER(8, sb); local ss = STROKE(C(35,35,55),1,0.6,sb)
        local sl = NEW("TextLabel",{
        ...
        sb.MouseButton1Click:Connect(function()
            getgenv().SelectedSea = sd.id
            GW_UpdateSeaSelection()
        end)
    end

    local function GW_UpdateUIState()
        local isReg = (getgenv().SelectedHub == "Regular")
        if gwSeaHeaderLbl then gwSeaHeaderLbl.Visible = isReg end
        for _,sd in ipairs(GW_SEA_OPTS) do
            if GW_SeaBtns[sd.id] then GW_SeaBtns[sd.id].Btn.Visible = isReg end
        end
    end

    -- Sync Config_SelectedHub/Sea with GW variants
    if not TogglesData["Config_SelectedHub"] then
        ...
        local oldHCB = TogglesData["Config_SelectedHub"].Callback
        TogglesData["Config_SelectedHub"].Callback = function(val)
            getgenv().SelectedHub = val or "Regular"
            GW_UpdateHubSelection()
            GW_UpdateUIState()
            if oldHCB then pcall(oldHCB, val) end
        end
    end
    ...
    -- Auto Join section — SMART: checks if already in PS before doing anything
    local gwAjY = gwSeaY + 62
    RowDivider(gwPsCard, gwAjY)
    NEW("TextLabel",{
        Text="AUTO JOIN",
        Size=UDim2.new(0.55,0,0,16), Position=UDim2.new(0,12,0,gwAjY+8),
        BackgroundTransparency=1, TextColor3=TEXT1,
        Font=Enum.Font.GothamBold, TextSize=12, TextXAlignment=Enum.TextXAlignment.Left
    }, gwPsCard)
    -- Smart status sub-label
    local gwAjSubLbl = NEW("TextLabel",{
        Text = isInPS
            and "Đang trong PS · toggle ON = lưu session để rejoin sau kick"
            or  "Toggle ON = lưu session rồi về lobby → lobby tự join PS",
        Size=UDim2.new(1,-24,0,12), Position=UDim2.new(0,12,0,gwAjY+26),
        BackgroundTransparency=1, TextColor3=TEXT3,
        Font=Enum.Font.Gotham, TextSize=9, TextXAlignment=Enum.TextXAlignment.Left,
        TextWrapped=true
    }, gwPsCard)

    -- Persistent auto-join toggle (same as lobby)
    local gwAjPill = NEW("TextButton",{
        Size=UDim2.new(0,48,0,26), Position=UDim2.new(1,-60,0,gwAjY+12),
        BackgroundColor3=BG5, Text="", AutoButtonColor=false
    }, gwPsCard)
    CORNER(20, gwAjPill)
    ...
    local gwAutoJoinActive = false
    local function GW_SetAutoJoin(on)
        gwAutoJoinActive = on
        getgenv().GBO_AutoJoin = on
        local ac, ad = COL_PS, PINKD
        TWEEN(gwAjPill,  0.22, {BackgroundColor3 = on and ad or BG5})
        TWEEN(gwAjStrk,  0.22, {Color = on and ac or TEXT3, Transparency = on and 0 or 0.3})
        TWEEN(gwAjThumb, 0.22, {
            BackgroundColor3 = on and ac or TEXT3,
            Position = on and UDim2.new(1,-22,0.5,-9) or UDim2.new(0,4,0.5,-9)
        })
        if TogglesData["Config_AutoJoinPS"] then
        ...
        if on then
            -- Save session immediately (for rejoin after kick)
            AutoRejoinModule._saveSession()
            -- If currently in a PUBLIC server → go to lobby right away so lobby auto-joins the PS
            local inPS = (game.PrivateServerId ~= "")
            if not inPS then
                task.spawn(function()
                    task.wait(0.6)
            ...
            -- If already in PS → do nothing extra (session saved, rejoin on kick is set)
        end
    end
    ...
    if not TogglesData["Config_AutoJoinPS"] then
        ...
        local oldCB = TogglesData["Config_AutoJoinPS"].Callback
        TogglesData["Config_AutoJoinPS"].Callback = function(val)
            GW_SetAutoJoin(val == true)
        end
    end
    gwAjPill.MouseButton1Click:Connect(function() GW_SetAutoJoin(not gwAutoJoinActive) end)

    -- JOIN NOW button (same as lobby — teleports to lobby if not in PS, else no-op)
    local gwJnY = gwAjY + 52
    local gwJoinNowBtn = NEW("TextButton",{
        Size=UDim2.new(1,-24,0,34), Position=UDim2.new(0,12,0,gwJnY),
        BackgroundColor3=PINKD, Text="", AutoButtonColor=false
    }, gwPsCard)
    CORNER(9, gwJoinNowBtn); STROKE(PINK,1.5,0.2,gwJoinNowBtn)
    DrawIcon(gwJoinNowBtn, "server", 14, 7, 20, PINK)
    NEW("TextLabel",{
        Text = isInPS and "REJOIN THIS SERVER" or "JOIN NOW",
        Size=UDim2.new(1,-46,0,16), Position=UDim2.new(0,40,0,8),
        BackgroundTransparency=1, TextColor3=PINK,
        Font=Enum.Font.GothamBold, TextSize=13, TextXAlignment=Enum.TextXAlignment.Left
    }, gwJoinNowBtn)
    NEW("TextLabel",{
        Text = isInPS and "Saves session + auto rejoin on kick" or "Saves session → về lobby → lobby tự join PS",
        Size=UDim2.new(1,-46,0,12), Position=UDim2.new(0,40,0,24),
        BackgroundTransparency=1, TextColor3=C(160,60,130),
        Font=Enum.Font.Gotham, TextSize=9, TextXAlignment=Enum.TextXAlignment.Left
    }, gwJoinNowBtn)

    gwJoinNowBtn.MouseEnter:Connect(function() TWEEN(gwJoinNowBtn,0.15,{BackgroundColor3=C(55,12,48)}) end)
    gwJoinNowBtn.MouseLeave:Connect(function() TWEEN(gwJoinNowBtn,0.15,{BackgroundColor3=PINKD}) end)
    gwJoinNowBtn.MouseButton1Click:Connect(function()
        TWEEN(gwJoinNowBtn,0.08,{BackgroundColor3=C(80,20,65)})
        task.wait(0.1)
        TWEEN(gwJoinNowBtn,0.15,{BackgroundColor3=PINKD})
        -- Always save session first
        AutoRejoinModule._saveSession()
        GW_SetAutoJoin(true)
        -- GW_SetAutoJoin already handles the teleport-to-lobby if in public
        -- If already in PS → session saved; nothing more to do
    end)

    -- Auto Rejoin toggle
    local gwArY = gwJnY + 44
    RowDivider(gwPsCard, gwArY)
    RowLabel(gwPsCard, "Auto Rejoin", "Saves session · rejoins after kick", gwArY+6)
    local gwArPill = NEW("TextButton",{
        Size=UDim2.new(0,48,0,26), Position=UDim2.new(1,-60,0,gwArY+10),
        BackgroundColor3=BG5, Text="", AutoButtonColor=false
    }, gwPsCard)
    CORNER(20, gwArPill)
    ...
    gwArPill.MouseButton1Click:Connect(function()
        ...
        TWEEN(gwArPill,  0.22, {BackgroundColor3 = on and CYAND or BG5})
        TWEEN(gwArStrk,  0.22, {Color = on and CYAN or TEXT3, Transparency = on and 0 or 0.3})
        TWEEN(gwArThumb, 0.22, {
            BackgroundColor3 = on and CYAN or TEXT3,
            Position = on and UDim2.new(1,-22,0.5,-9) or UDim2.new(0,4,0.5,-9)
        })
        if d.Callback then d.Callback(on) end
    end)

    -- Resize gwPsCard to fit
    task.spawn(function()
        task.wait(0.05)
        gwPsCard.Size = UDim2.new(1,-24, 0, gwArY + 46)
        GW_UpdateHubSelection()
        GW_UpdateSeaSelection()
        GW_UpdateUIState()
    end)
    ...
    local maccData  = {} -- { [accName] = {code, hub, sea, config} }

    local function MAccSave()
        pcall(function()
            if not writefile then return end
            local ok,js = pcall(function()
                return HttpService:JSONEncode(maccData)
            end)
    ...
    local function MAccLoad()
        pcall(function()
            if isfile and isfile(MACC_FILE) then
                local ok,d = pcall(function()
                    return HttpService:JSONDecode(readfile(MACC_FILE))
                end)
    ...
    NEW("TextLabel",{
        Text="Mỗi acc tự load PS + config khi vào lobby  ·  Config tự set trong tab Config",
    ...
    -- ── Entry list (scrolling, fixed height) ────────────────────────────
    ...
    CORNER(8, maccScroll)
    STROKE(C(70,30,110), 1.5, 0.2, maccScroll)
    NEW("UIListLayout",{
        SortOrder=Enum.SortOrder.LayoutOrder,
    ...
    local maccRows = {}
    local maccSelected = nil

    local function MAccRefreshList()
        for _,r in ipairs(maccRows) do if r.frame and r.frame.Parent then r.frame:Destroy() end end
        -- Clear any orphan TextLabels (empty state)
        ...
        for accName, slot in pairs(maccData) do
            idx = idx + 1
            local isSel = (maccSelected == accName)
            local row = NEW("Frame",{
                Size=UDim2.new(1,-4,0,50), BackgroundColor3= isSel and C(22,8,44) or C(14,11,30),
                LayoutOrder=idx, BorderSizePixel=0
            }, maccScroll)
            CORNER(7, row)
            local rowStrk = STROKE(isSel and PURPLE or C(55,30,90), isSel and 1.5 or 1, isSel and 0 or 0.4, row)
            ...
            NEW("TextLabel",{
                Text="👤  "..accName,
                Size=UDim2.new(1,-90,0,20), Position=UDim2.new(0,xOff,0,4),
                BackgroundTransparency=1,
                TextColor3 = isSel and GOLD2 or TEXT1,
                Font=Enum.Font.GothamBold, TextSize=12,
                TextXAlignment=Enum.TextXAlignment.Left
            }, row)

            -- Detail: hub · sea · config · code snippet
            local hub    = slot.hub    or "Regular"
            local sea    = slot.sea    or "Sea 1"
            local config = slot.config and slot.config ~= "" and slot.config or "—"
            local code   = slot.code   or ""
            local codeShow = code=="" and "Public" or (string.sub(code,1,8).."…")
            NEW("TextLabel",{
                Text=hub.."  ·  "..sea.."  ·  "..codeShow.."  ·  cfg: "..config,
                Size=UDim2.new(1,-90,0,16), Position=UDim2.new(0,xOff,0,26),
                BackgroundTransparency=1,
                TextColor3 = isSel and C(180,140,240) or C(130,115,160),
            ...
            local selBtn = NEW("TextButton",{
                ...
                Size=UDim2.new(0,78,0,32), Position=UDim2.new(1,-84,0.5,-16),
                BackgroundColor3= isSel and C(30,10,58) or C(18,8,36),
                TextColor3 = isSel and GOLD2 or PURPLE,
                Font=Enum.Font.GothamBold, TextSize=9, AutoButtonColor=false
            }, row)
            CORNER(7, selBtn)
            STROKE(isSel and GOLD2 or PURPLE, 1.2, isSel and 0.1 or 0.35, selBtn)

            selBtn.MouseButton1Click:Connect(function()
                maccSelected = (maccSelected==accName) and nil or accName
                -- Fill input boxes with selected slot data
                if maccSelected == accName then
                    maccNameBox.Text = accName
                    maccCodeBox.Text = code
                    maccCfgBox.Text  = slot.config or ""
                    -- Set hub cycle
                    for i,h in ipairs(maccHubCycle) do
                        if h == hub then maccHubIdx = i; maccHubBtn.Text = h break end
                    end
                    -- Set sea
                    maccSeaState = sea
                    maccSeaBtn.Text = sea == "Sea 1" and "SEA 1" or "SEA 2"
                end
                MAccRefreshList()
            end)
            selBtn.MouseEnter:Connect(function()
                if maccSelected ~= accName then
                    TWEEN(selBtn,0.1,{BackgroundColor3=C(28,12,52)})
                end
            end)
            selBtn.MouseLeave:Connect(function()
                if maccSelected ~= accName then
                    TWEEN(selBtn,0.1,{BackgroundColor3=C(18,8,36)})
                end
            end)

            table.insert(maccRows, {frame=row, nameLbl=nil})
        end
        if idx == 0 then
            local emptyLbl = NEW("TextLabel",{
                Text="Chưa có tài khoản nào · nhập bên dưới và nhấn ADD",
    ...
    -- ADD area ─────────────────────────────────────────────────────────────
    local addY = 196  -- 58 (scroll top) + 130 (scroll h) + 8 gap
    ...
    -- Row 1: AccName + PS code
    local maccNameBg = NEW("Frame",{
        Size=UDim2.new(0.42,-6,0,28), Position=UDim2.new(0,12,0,addY+22),
        BackgroundColor3=C(12,10,28), BorderSizePixel=0
    }, maccCard); CORNER(6,maccNameBg); STROKE(C(80,50,120),1.5,0.2,maccNameBg)
    local maccNameBox = NEW("TextBox",{
        Size=UDim2.new(1,-10,1,0),Position=UDim2.new(0,5,0,0),BackgroundTransparency=1,
        Text="",PlaceholderText="Tên account...",
        TextColor3=C(235,225,255),PlaceholderColor3=C(90,75,120),
        Font=Enum.Font.GothamSemibold,TextSize=11,
        ClearTextOnFocus=false
    }, maccNameBg)

    local maccCodeBg = NEW("Frame",{
        Size=UDim2.new(0.58,-6,0,28), Position=UDim2.new(0.42,0,0,addY+22),
        BackgroundColor3=C(12,10,28), BorderSizePixel=0
    }, maccCard); CORNER(6,maccCodeBg); STROKE(C(80,50,120),1.5,0.2,maccCodeBg)
    local maccCodeBox = NEW("TextBox",{
        Size=UDim2.new(1,-10,1,0),Position=UDim2.new(0,5,0,0),BackgroundTransparency=1,
        Text="",PlaceholderText="PS code (trống = public)...",
        TextColor3=C(235,225,255),PlaceholderColor3=C(90,75,120),
        Font=Enum.Font.Gotham,TextSize=10,
        ClearTextOnFocus=false
    }, maccCodeBg)

    -- Row 2: Hub cycle + Sea toggle + Config name
    local maccHubCycle = {"Regular","Trade Hub","Universe Hub","Fish Hub"}
    local maccHubIdx   = 1
    local maccHubBtn = NEW("TextButton",{
    ...
    local maccSeaBtn = NEW("TextButton",{
        Text="SEA 1",
    ...
    local maccCfgBg = NEW("Frame",{
        Size=UDim2.new(0.52,-4,0,26), Position=UDim2.new(0.48,0,0,addY+54),
        BackgroundColor3=C(12,10,28), BorderSizePixel=0
    }, maccCard); CORNER(6,maccCfgBg); STROKE(C(80,50,120),1.5,0.2,maccCfgBg)
    local maccCfgBox = NEW("TextBox",{
        Size=UDim2.new(1,-10,1,0),Position=UDim2.new(0,5,0,0),BackgroundTransparency=1,
        Text="",PlaceholderText="Tên config...",
        TextColor3=C(235,225,255),PlaceholderColor3=C(90,75,120),
        Font=Enum.Font.Gotham,TextSize=10,
        ClearTextOnFocus=false
    }, maccCfgBg)
    ...
    local maccAddBtn = NEW("TextButton",{
        Text="✚  ADD / UPDATE",
    ...
    }, maccCard); CORNER(7,maccAddBtn); STROKE(PURPLE,1.5,0.1,maccAddBtn)

    local maccDelBtn = NEW("TextButton",{
        Text="✕  REMOVE",
        Size=UDim2.new(0.45,-6,0,30), Position=UDim2.new(0.55,0,0,addY+84),
        BackgroundColor3=C(38,8,8), TextColor3=C(255,100,100),
        Font=Enum.Font.GothamBold, TextSize=11, AutoButtonColor=false
    }, maccCard); CORNER(7,maccDelBtn); STROKE(RED,1.5,0.2,maccDelBtn)

    maccAddBtn.MouseEnter:Connect(function() TWEEN(maccAddBtn,0.12,{BackgroundColor3=C(36,14,66)}) end)
    maccAddBtn.MouseLeave:Connect(function() TWEEN(maccAddBtn,0.12,{BackgroundColor3=C(24,10,48)}) end)
    maccDelBtn.MouseEnter:Connect(function() TWEEN(maccDelBtn,0.12,{BackgroundColor3=C(58,12,12)}) end)
    maccDelBtn.MouseLeave:Connect(function() TWEEN(maccDelBtn,0.12,{BackgroundColor3=C(38,8,8)}) end)

    maccAddBtn.MouseButton1Click:Connect(function()
        ...
        MAccSave()
        MAccRefreshList()
        TWEEN(maccAddBtn, 0.1, {TextColor3=GREEN})
        task.delay(1.2, function()
            if maccAddBtn and maccAddBtn.Parent then
                TWEEN(maccAddBtn,0.3,{TextColor3=C(200,160,255)})
            end
        end)
    end)

    maccDelBtn.MouseButton1Click:Connect(function()
        if not maccSelected then return end
        maccData[maccSelected] = nil
        maccSelected = nil
        MAccSave()
        MAccRefreshList()
    end)

    -- Helper exported to global so lobby can call it on startup
    getgenv().GBO_MAccData = maccData -- shared reference; lobby reads this
    getgenv().GBO_MAccApply = function(accName)
        local slot = maccData[accName]
        if not slot then return false end
        -- Set PS code
        getgenv().PSCode = slot.code or ""
        pcall(function()
            if gwPsBox then gwPsBox.Text = getgenv().PSCode end
        end)
        -- Set hub + sea
        getgenv().SelectedHub = slot.hub or "Regular"
        getgenv().SelectedSea = slot.sea or "Sea 1"
        GW_UpdateHubSelection()
        GW_UpdateSeaSelection()
        return true
    end
